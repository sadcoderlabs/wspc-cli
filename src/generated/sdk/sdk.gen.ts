 ...
 * ### Constraints
 * - Public endpoint (no authentication required).
 * - Both discovery paths share a limit of 30 requests per IP per 60 seconds, separate from client registration. A 429 response includes Retry-After: 60.
 ...
 *
 * ### When to Use
 * - Shares the 30 requests per IP per 60 seconds limit with the RFC 8414 path. Client registration has a separate limit. A 429 response includes Retry-After: 60.
 ...
 * For the `refresh_token` grant, `error_description` names which one it was, since `error` is always `invalid_grant`: `refresh_token_unknown`, `refresh_token_expired`, `refresh_token_revoked`, `refresh_token_reused`, `refresh_token_client_mismatch`, `refresh_token_resource_mismatch`.
 * `refresh_token_reused` means a token that had already been rotated was presented again outside the tolerance window, so the whole token family was revoked as a possible replay — every session on that credential must sign in again. Treat it as a signal that something is holding a stale copy of the credentials, not as ordinary expiry.
 ...
 */
export const billingEmailUsageGet = <ThrowOnError extends boolean = false>(options?: Options<BillingEmailUsageGetData, ThrowOnError>) => (options?.client ?? client).get<BillingEmailUsageGetResponses, BillingEmailUsageGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Get Current Workspace enforcement migration readiness
 */
export const billingEnforcementReadinessGet = <ThrowOnError extends boolean = false>(options?: Options<BillingEnforcementReadinessGetData, ThrowOnError>) => (options?.client ?? client).get<BillingEnforcementReadinessGetResponses, BillingEnforcementReadinessGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Get Current Workspace inbound protection state
 */
export const billingInboundProtectionGet = <ThrowOnError extends boolean = false>(options?: Options<BillingInboundProtectionGetData, ThrowOnError>) => (options?.client ?? client).get<BillingInboundProtectionGetResponses, BillingInboundProtectionGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Get Current Workspace retained storage usage
 */
export const billingStorageUsageGet = <ThrowOnError extends boolean = false>(options?: Options<BillingStorageUsageGetData, ThrowOnError>) => (options?.client ?? client).get<BillingStorageUsageGetResponses, BillingStorageUsageGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Disable the private calendar link
 *
 * Disable the link using expected_version. External copies cannot be removed. A private read-only link to all your event details in the current Workspace. Anyone with the link can read them. External calendar updates depend on that app's refresh schedule and can be delayed.
 */
export const calendarDisableSubscription = <ThrowOnError extends boolean = false>(options: Options<CalendarDisableSubscriptionData, ThrowOnError>) => (options.client ?? client).delete<CalendarDisableSubscriptionResponses, CalendarDisableSubscriptionErrors, ThrowOnError>({
...
/**
 * Read the private calendar subscription
 *
 * Read the current subscription state without creating a link. A private read-only link to all your event details in the current Workspace. Anyone with the link can read them. External calendar updates depend on that app's refresh schedule and can be delayed.
 */
export const calendarGetSubscription = <ThrowOnError extends boolean = false>(options?: Options<CalendarGetSubscriptionData, ThrowOnError>) => (options?.client ?? client).get<CalendarGetSubscriptionResponses, CalendarGetSubscriptionErrors, ThrowOnError>({ url: '/calendar/subscription', ...options });

/**
 * Create a private calendar subscription
 *
 * Create a subscription or return the same active link. A private read-only link to all your event details in the current Workspace. Anyone with the link can read them. External calendar updates depend on that app's refresh schedule and can be delayed.
 */
export const calendarCreateSubscription = <ThrowOnError extends boolean = false>(options?: Options<CalendarCreateSubscriptionData, ThrowOnError>) => (options?.client ?? client).post<CalendarCreateSubscriptionResponses, CalendarCreateSubscriptionErrors, ThrowOnError>({ url: '/calendar/subscription', ...options });
...
/**
 * List a bounded calendar agenda
 *
 ...
 */
export const eventAgenda = <ThrowOnError extends boolean = false>(options: Options<EventAgendaData, ThrowOnError>) => (options.client ?? client).get<EventAgendaResponses, EventAgendaErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/calendar/agenda',
    ...options
});
...
/**
 * List occurrences of one recurring series
 *
 ...
 */
export const eventOccurrences = <ThrowOnError extends boolean = false>(options: Options<EventOccurrencesData, ThrowOnError>) => (options.client ?? client).get<EventOccurrencesResponses, EventOccurrencesErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Reset the private calendar link
 *
 * Replace the link using expected_version. The old link stops working; external copies cannot be removed. Subscribe again with the new link. After an uncertain response, read the current state before retrying. A private read-only link to all your event details in the current Workspace. Anyone with the link can read them. External calendar updates depend on that app's refresh schedule and can be delayed.
 */
export const calendarResetSubscription = <ThrowOnError extends boolean = false>(options: Options<CalendarResetSubscriptionData, ThrowOnError>) => (options.client ?? client).post<CalendarResetSubscriptionResponses, CalendarResetSubscriptionErrors, ThrowOnError>({
...
/**
 * Cancel the in-flight Drive Export Job
 *
 * Cancels the Workspace's unfinished Export Job so a new one can be started, and returns the latest job (or null when there was nothing to cancel).
 */
export const driveExportCancel = <ThrowOnError extends boolean = false>(options?: Options<DriveExportCancelData, ThrowOnError>) => (options?.client ?? client).post<DriveExportCancelResponses, DriveExportCancelErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Get the latest Drive Export Job
 *
 * Export Packages hold every current file version in the Workspace, one folder per library. Version history and trashed items are left out, and the package does not count against the Workspace storage quota. Packages are deleted 7 days after they complete; the job record then reports `expired`. Concurrent edits are observed batch by batch, not as a point-in-time snapshot. A missing selected source fails the job with source_object_missing; unrecoverable packing errors use packing_failed. Create a new job for these guarantees; earlier packages retain their original expiry. Returns workspace_id and a null job when the Workspace has never exported.
 */
export const driveExportGet = <ThrowOnError extends boolean = false>(options?: Options<DriveExportGetData, ThrowOnError>) => (options?.client ?? client).get<DriveExportGetResponses, DriveExportGetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/drive/export',
    ...options
});
...
/**
 * Start a Drive Export Job
 *
 * Export Packages hold every current file version in the Workspace, one folder per library. Version history and trashed items are left out, and the package does not count against the Workspace storage quota. Packages are deleted 7 days after they complete; the job record then reports `expired`. Concurrent edits are observed batch by batch, not as a point-in-time snapshot. A missing selected source fails the job with source_object_missing; unrecoverable packing errors use packing_failed. Create a new job for these guarantees; earlier packages retain their original expiry. Returns 409 DRIVE_EXPORT_IN_PROGRESS when the Workspace already has an unfinished job.
 */
export const driveExportCreate = <ThrowOnError extends boolean = false>(options?: Options<DriveExportCreateData, ThrowOnError>) => (options?.client ?? client).post<DriveExportCreateResponses, DriveExportCreateErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/drive/export',
    ...options
});
...
/**
 * Download the completed Export Package
 *
 * Streams the authenticated Workspace's uncompressed tar. Supply job_id and workspace_id together to download that immutable package; omit both for legacy latest behavior. Invalid pairs return VALIDATION_ERROR, a wrong Workspace returns WORKSPACE_FORBIDDEN, and an unknown scoped job returns NOT_FOUND. Unfinished, expired, missing, or size-mismatched packages return DRIVE_EXPORT_NOT_READY. New jobs do not shorten earlier packages' retention. The stream has no content encoding.
 */
export const driveExportDownload = <ThrowOnError extends boolean = false>(options?: Options<DriveExportDownloadData, ThrowOnError>) => (options?.client ?? client).get<DriveExportDownloadResponses, DriveExportDownloadErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 * Move a drive file
 *
 * Move the confirmed entry and version in place. Preserve content and history. Never replace the original confirmation with current values.
 */
...
/**
 * Restore a deleted file from its latest retained version
 *
 * Restores the confirmed deleted entry at its original path from its latest retained version. Requires entry_id, path, and a positive safe-integer expected_entry_version. Creates an independent content version, increases Entry Version once, and returns the committed entry snapshot with result updated. Retained objects remain billable. An occupied path is not overwritten. An old confirmation returns VERSION_CONFLICT. If the response is lost, the commit outcome is unknown; read the original identity before deciding what to do and do not retry automatically.
 */
export const driveFileRestoreDeleted = <ThrowOnError extends boolean = false>(options: Options<DriveFileRestoreDeletedData, ThrowOnError>) => (options.client ?? client).post<DriveFileRestoreDeletedResponses, DriveFileRestoreDeletedErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Move sent messages to trash
 *
 * Soft-deletes current-user sent messages so they remain restorable during retention.
 */
export const emailSentDelete = <ThrowOnError extends boolean = false>(options: Options<EmailSentDeleteData, ThrowOnError>) => (options.client ?? client).post<EmailSentDeleteResponses, EmailSentDeleteErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Permanently purge email trash before a fixed cutoff
 *
 * OAuth-only destructive operation. Pass the fixed deletion cutoff returned by the caller confirmation flow.
 */
export const emailTrashEmpty = <ThrowOnError extends boolean = false>(options: Options<EmailTrashEmptyData, ThrowOnError>) => (options.client ?? client).post<EmailTrashEmptyResponses, EmailTrashEmptyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 * List sent email
 *
 * Lists the current user's non-purged sent messages, with optional trash filters.
 */
export const emailSentList = <ThrowOnError extends boolean = false>(options?: Options<EmailSentListData, ThrowOnError>) => (options?.client ?? client).get<EmailSentListResponses, EmailSentListErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Restore sent messages from trash
 *
 * Restores current-user sent messages that have not been permanently purged.
 */
export const emailSentRestore = <ThrowOnError extends boolean = false>(options: Options<EmailSentRestoreData, ThrowOnError>) => (options.client ?? client).post<EmailSentRestoreResponses, EmailSentRestoreErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Get email trash summary
 *
 * Returns current-user inbound and sent trash counts, bytes, and earliest expiry.
 */
export const emailTrashSummary = <ThrowOnError extends boolean = false>(options?: Options<EmailTrashSummaryData, ThrowOnError>) => (options?.client ?? client).get<EmailTrashSummaryResponses, EmailTrashSummaryErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
 ...
 *
 * ### 💡 Key Features & Constraints
 * * **Fixed Assignee**: Any PATCH with `assignee_user_id`, including the same value or null, fails with `ASSIGNEE_IMMUTABLE` (HTTP 400).
 * * **Rebuild**: Rule edits rebuild open instances from today whose root Todo is unmodified. Membership must be valid before clearing instances; otherwise edits save and rebuilding is deferred.
