...
`todo-done.ts` 改用同一個 helper，保留 `TODO_UPDATE_DISPLAY`。`email/send.ts` 在完成 text、reply、attachment validation 與 body construction 後，使用 helper 呼叫 `emailSend`，並用 data selector 選出 `result.data!.email` 的等價輸出。
...
- `email send` 的 validation、attachment limit、success render 與 HTTP error behavior 不變。
