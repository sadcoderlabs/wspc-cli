...
- **持續被拒的組合每輪都會重抓一次 full manifest**：略過該組的那一輪仍會寫入新 cursor，下一輪又是 delta，於是再次被拒、再抓一次 full manifest，每輪多出兩次 move 請求與一次完整清單分頁。只有 server 對同一份確認持續拒絕時才會發生（例如非 409 的 4xx）；這個代價可接受，換來的是不會永久卡住。
