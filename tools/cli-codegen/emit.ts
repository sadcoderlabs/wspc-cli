...
export function emitCommand(input: EmitInput): string | null {
  ...
  function resolveAlias(fieldName: string): {
    ...
    if (aliases[fieldName] !== undefined)
      return { longFlag: flagName, short: aliases[fieldName] };
    if (aliases[flagName] !== undefined)
      return { longFlag: flagName, short: aliases[flagName] };
    // Prefix match: alias key "project" covers "project_id" / "project-id"
    for (const [aliasKey, short] of Object.entries(aliases)) {
      if (
        fieldName.startsWith(aliasKey + "_") ||
        flagName.startsWith(aliasKey + "-")
      ) {
  ...
  const hasAttendeeParser = Object.values(xCliOptions).some(
    (o) => o.parser === "attendee",
  );
  ...
  const args: string[] = positional.map((name) => {
    ...
    const field = isPathParam
      ? undefined
      : input.bodyFields.find((f) => f.name === name);
  ...
  function emitFieldOption(f: BodyField): string {
    ...
    if (optKey !== undefined) {
      ...
      const flagSpec = short
        ? `-${short}, --${longFlag} <value>`
        : `--${longFlag} <value>`;
  ...
  for (const [optKey, optDef] of Object.entries(xCliOptions)) {
    if (optDef.parser === "datetime") {
      ...
      if (optDef.allDayFlag) {
        ...
        if (optDef.exclusive) {
          conversionLines.push(
            `        ${valueVar} = inclusiveEndToExclusive(opts.${camelKey} as string)`,
          );
        } else {
          conversionLines.push(
            `        ${valueVar} = parseDateOnly(opts.${camelKey} as string)`,
          );
        }
      ...
      const source = variadicPositionalSet.has(optKey)
        ? camelKey
        : `opts.${camelKey}`;
      conversionLines.push(
  ...
  if (exitOnField) {
    const pathParts = (exitOnField.path || "")
      .split(".")
      .filter((p) => p.trim() !== "");
    const accessExpr =
      pathParts.length > 0 ? `data?.${pathParts.join("?.")}` : `data`;
    exitLines.push(
